...
class xt_novalnet_sepa
{
    ...
    function xt_novalnet_sepa()
    {
        ...
        $this->data['js_path']            = _SRV_WEB_PLUGINS.'xt_novalnet_sepa/javascript/xt_novalnet_sepa.js';
        $this->data['js_date_picker_path']= _SRV_WEB_PLUGINS.'xt_novalnet_sepa/javascript/datepicker-'.$_SESSION['selected_language'].'.js';
    ...
    function additional_parameters($xt_novalnet_config, &$parameters) {
		...
		$sepa_due_date = XT_NOVALNET_SEPA_DUEDATE;
		$parameters['sepa_due_date']          = ( $sepa_due_date != '' ) ? date('Y-m-d', strtotime('+' . $sepa_due_date . 'days')) : date('Y-m-d', strtotime('+7 days'));
