...
class callback_xt_novalnet_config extends callback
{
   ...
   protected $technic_notify_mail = 'technic@novalnet.de';
    ...
    function process()
    {
		...
		if ($server_request['payment_type'] == 'TRANSACTION_CANCELLATION' && !in_array($this->server_request['current_order_previous_status'], array('75','91','99'))) {
	...
	function zero_level_process()
	{
		...
		if (in_array($this->server_request['payment_type'], array('PAYPAL', 'PRZELEWY24')) && $this->server_request['tid_status'] == '100' && $this->server_request['status'] == '100'){
		...
		} else if(in_array($this->server_request['payment_type'],array('GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA','INVOICE_START','DIRECT_DEBIT_SEPA')) && in_array($this->server_request['tid_status'], array(91,99,100)) && $this->server_request['status'] == 100 && in_array($this->transaction_history['current_order_previous_status'] ,array(75,91,99))) {
			$comments = '';
			$test_mode_value         = ($this->server_request['test_mode'] == '1');
			$comments                .= self::form_transaction_comments($this->server_request, $test_mode_value);
			if( in_array($this->server_request['tid_status'],array(99,91)) && $this->transaction_history['current_order_previous_status'] == 75){
				...
				$comments .= $callback_comments;
			} else if($this->server_request['tid_status'] == 100 && in_array( $this->transaction_history['current_order_previous_status'], array(75,91,99))){
				...
				if(in_array($this->server_request['payment_type'],array('GUARANTEED_INVOICE','INVOICE_START')) && in_array($this->transaction_history['current_order_previous_status'], array(75, 91))){
					...
					$comments .= Novalnet::form_invoice_prepayment_comments($request, $this->server_request['product_id'], $this->server_request['payment_type'], false);
			...
			$this->send_notify_mail(
				...
				//payment failure
				$callback_comments = XT_NOVALNET_MENTION_PAYMENT_CATEGORY_TEXT.'<br>';
				$callback_comments .= XT_NOVALNET_CALLBACK_SCRIPT_CANCEL_TEXT;
				$callback_comments .= isset($this->server_request['status_text']) ? $this->server_request['status_text'] : (isset($this->server_request['status_desc']) ? $this->server_request['status_desc'] : (isset($this->server_request['status_message']) ? $this->server_request['status_message'] : ''));
				// Update callback comments in order status history table
	...
	function first_level_process()
	{
		...
		if (in_array($this->server_request['payment_type'], $this->chargebacks) && $this->server_request['status'] == '100' && $this->server_request['tid_status'] == '100') {
			...
			$message = (in_array($this->server_request['payment_type'], array('CREDITCARD_BOOKBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND','CASHPAYMENT_REFUND','GUARANTEED_SEPA_BOOKBACK','GUARANTEED_INVOICE_BOOKBACK'))) ? XT_NOVALNET_CALLBACK_SCRIPT_BOOKBACK_TEXT : XT_NOVALNET_CALLBACK_SCRIPT_CHARGEBACK_TEXT;

			$callback_comments = sprintf($message, $this->server_request['tid_payment'], $this->received_amount, date_short(date('Y-m-d')), date('H:i:s'), $this->server_request['tid']);
					...
					if ($this->transaction_history['org_total'] <= ($total_paid_amount['amount_total'] + $this->server_request['amount'])) {
						$callback_secound_level_payment_execute = false;
							if($this->server_request['payment_type'] == 'ONLINE_TRANSFER_CREDIT' ){
								$callback_comments .= '<br>'.sprintf(XT_NOVALNET_CALLBACK_ONLINE_TRANFER_CREDIT_EXTRA_TEXT, $this->received_amount, $this->order_id );
							}
						// Full Payment paid
    ...
    {
        if (XT_NOVALNET_CALLBACK_MAIL_SEND == 'true' && !empty($data)) {
            ...
            if (!empty($email_to) && filter_var($email_to, FILTER_VALIDATE_EMAIL)) {
                ...
                $exportMail->_setSubject(_STORE_NAME . 'Novalnet Vendor script notification');

                if (!empty(XT_NOVALNET_CALLBACK_MAIL_BCC) && filter_var(XT_NOVALNET_CALLBACK_MAIL_BCC, FILTER_VALIDATE_EMAIL)) {
                    $exportMail->_addBCC(XT_NOVALNET_CALLBACK_MAIL_BCC, '');
                }
    ...
    function get_order_reference()
    {
        ...
        if ($order_no) {
            ...
            if (empty($this->noval_order->order_data['customers_id'])) {
    ...
    function handle_communication_failure()
    {
        ...
        if (isset($this->noval_order->order_data['customers_id'])) {
			...
			$order_status = defined($status_code) ? constant($status_code) : '';
			// Form Transaction comments
    ...
    function build_notification_mail() {
        ...
        $subject = sprintf(' Critical error on shop system %s order not found for TID: %s',_STORE_NAME, $this->server_request['shop_tid']);
        $message = 'Dear Technic team,<br><br>&emsp;&emsp;Please evaluate this transaction and contact our payment module team at Novalnet<br><br>';
        $params = array(
        ...
        foreach($params as $key => $value) {
            if (!empty($this->server_request[$key])) {
                $message .= "$value: " . $this->server_request[$key] . '<br>';
            }
        }
        $message .= '<br>Regards,<br>Novalnet Team';
