...
class xt_novalnet_operations
{
    ...
    public function manageTransactionProcess($request){
        ...
        if ($response['status'] == '100') {
            ...
            if ($request['novalnet_transaction_status_change'] == '100') {
                ...
                $comments = (in_array($transaction_details['payment_id'],array('27','41')) && $response['due_date'] != '') ? sprintf(XT_NOVALNET_TRANSACTION_CONFIRM_INVOICE_MESSAGE, $transaction_details['tid'], $response['due_date']).'<br>' : sprintf(XT_NOVALNET_TRANSACTION_CONFIRM_MESSAGE_TEXT, date_short(date('Y-m-d')), date('H:i:s')).'<br>';
                ...
                $comments = sprintf(XT_NOVALNET_TRANSACTION_CANCEL_MESSAGE_TEXT, date_short(date('Y-m-d')), date('H:i:s'));
            }
            // General transaction comments
            $comments .= '<br>'.Novalnet::form_transaction_comments($transaction_details, $transaction_details['test_mode']);
    ...
    public function amountBookProcess($request){
        ...
        if ($response['status'] == '100') {
            ...
            $comments     = Novalnet::form_transaction_comments($response, $test_mode);
    ...
    public function refundProcess($request){
        ...
        if ($response['status'] == '100') {
            ...
            if (!empty($response['tid'])) {
                $comments .= XT_NOVALNET_REFUND_NEW_TID_MESSAGE_TEXT . $response['tid'];
            } elseif (!empty($response ['paypal_refund_tid'])) {
                $comments .= XT_NOVALNET_REFUND_NEW_TID_MESSAGE_TEXT . $response ['paypal_refund_tid'];
            }
    ...
    public function amountUpdateProcess($request){
        ...
        if ($response['status'] == '100') {
            ...
            if (in_array($transaction_details['payment_id'],array('59','27','41'))) {
                ...
                // General transaction comments
                $comments .= '<br>'.Novalnet::form_transaction_comments($transaction_details, $test_mode);
