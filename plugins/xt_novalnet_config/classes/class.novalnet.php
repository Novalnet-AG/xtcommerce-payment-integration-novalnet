...
class Novalnet
{
	...
	private static $_supports_shopping_type = array( 'xt_novalnet_cc', 'xt_novalnet_sepa', 'xt_novalnet_paypal' );
    ...
    public static function perform_curlrequest( $parameters, $url = 'https://payport.novalnet.de/paygate.jsp'  )
    {
        ...
        $gateway_timeout = trim(XT_NOVALNET_GATEWAY_TIMEOUT);
        $gateway_timeout = ($gateway_timeout != '') ? $gateway_timeout : '240';
        // Proxy configuration
        $proxy_server = trim(XT_NOVALNET_PROXY_SERVER);
        ...
        // Set proxy
        if ($proxy_server != '') {
            curl_setopt($curl_process, CURLOPT_PROXY, $proxy_server);
        }
        // Execute cURL
    ...
    public static function form_payment_params( $selected_payment )
    {
        ...
        $referrer_id       = trim(XT_NOVALNET_REFERRER_ID);
        ...
        $additional_parameters = array(
			...
			'referrer_id' => is_numeric($referrer_id) ? $referrer_id : '',
        ...
        $parameters = array_merge(
            ...
            array(
             ...
             'system_name'      => 'xtcommercestart',
             'system_version'   => _SYSTEM_VERSION.'-NN 11.2.0',
    ...
    public static function form_transaction_comments( $response, $test_mode, $payment_type = null  )
    {
		$comments = '';
		if ($response['tid']) {
			$comments .= XT_NOVALNET_TRANSACTION_DETAILS_TEXT.'<br>';
    ...
    public static function form_invoice_prepayment_comments( $response, $product_id, $selected_payment )
    {
		...
		if($response['tid_status'] != '75'){
			...
			$trans_comments .= XT_NOVALNET_TRANSFER_INFO_TEXT.'<br/>';
			if (!empty($response['due_date'])) {
				$trans_comments .= XT_NOVALNET_DUE_DATE_TEXT.': '.date_short($response['due_date']).'<br/>';
			}
			$order_no     = !empty($response['order_no']) ? $response['order_no'] : $response['orders_id'];
			$order_amount = (!empty($response['formated_amount'])) ? $response['formated_amount'] :  $_SESSION['cart']->total['formated'];
			...
			$trans_comments .= XT_NOVALNET_AMOUNT_TEXT.': '.$order_amount.'<br/><br/>';

			// Form payment reference comments
			$references = array_filter(
				array(
				 'BNR-'.$product_id.'-'.$order_no               => 'payment_reference1',
				 'TID '.$response['tid']                        => 'payment_reference2',
				)
			);
        ...
        if (isset($response['status']) && ($response['status'] == '100' || ($selected_payment == 'xt_novalnet_paypal' && $response['status'] == '90'))) {
            ...
            if (in_array($selected_payment, array( 'xt_novalnet_invoice', 'xt_novalnet_prepayment' ))) {
			   ...
			   $paid_amount  = 0;
				...
				$paid_amount  = 0;
				$comments .= XT_NOVALNET_CASHPAYMENT_DUE_DATE_FIELD_TEXT.': '.date_short($response['cashpayment_due_date']).'<br/>';
            ...
            if (in_array($response['tid_status'], array('90', '86'))) {
				...
				$order_status_id = constant(strtoupper($selected_payment).'_CALLBACK_ORDER_STATUS');
			}
			...
			if (in_array($selected_payment, self::$_supports_shopping_type)) {
				...
				// Handle one click process
					...
					if ($selected_payment == 'xt_novalnet_cc' && empty($_SESSION['xt_novalnet_cc_data']['xt_novalnet_cc_one_click_process']) && XT_NOVALNET_CC_ENABLE_CC3D != 'true' && XT_NOVALNET_CC_ENABLE_CC3D_FORCE != 'true') {
						$payment_details = json_encode(
						   array(
            ...
            $transaction_table_values = array(
				...
				'order_no'        => $response['order_no'],
				'due_date'        => !empty($response['due_date']) ? $response['due_date'] : (!empty($response['cashpayment_due_date']) ? $response['cashpayment_due_date'] : ''),
				'date'            => date('Y-m-d H:i:s'),
            ...
            // Update the formed values to the tables
            self::update_tables($response, $callback_table_values, $transaction_table_values);
			$tid       = !empty($response['tid']) ? $response['tid'] : '';
    ...
    public static function redirect_parameters( &$parameters, $code )
    {
        ...
        $parameters['error_return_url'] = $parameters['return_url'] = str_replace(
            '&amp;', '&', $xtLink->_link(
                array(
                'page'   => 'checkout',
                'params' => "page_action=payment_process&novalnet_payment=$code",
                'conn'   => 'SSL'
                )
            )
        );
     ...
     * @return array
     */
    public static function perform_xml_request($request_parameters)
    {
		$request_parameters['remote_ip'] = self::get_client_ip();
        ...
        return json_decode( json_encode( (array) simplexml_load_string( self::perform_curlrequest($parameters, 'https://payport.novalnet.de/nn_infoport.xml') ) ), 1 );
    }
     ...
     *
     */
    public static function get_fraud_module_details(&$data, $code)
    {
		global $xtPlugin;
		$data['enabled_fraud_module'] = constant(strtoupper($code) . '_ENABLE_FRAUD_PREVENTION');
		$_SESSION[$code]['fraud_module_enabled'] = false;
		$data['show_fraud_module'] = false;
		if (!empty($xtPlugin->active_modules['xt_novalnet_config']) && $data['enabled_fraud_module'] != 'none') {

			$minimum_amount = (int) trim(constant(strtoupper($code) . '_MIN_FRAUD_PREVENTION_AMOUNT'));
			$order_amount   = (int) self::get_server_amount_format($_SESSION['cart']->total['plain']);

			if (in_array($_SESSION['customer']->customer_default_address['customers_country_code'], self::$_allowed_countries) && (empty($minimum_amount) || ($order_amount >= $minimum_amount))) {

				// Assign label and default value for input field
				if ($data['enabled_fraud_module'] == 'SMS') {
					$data['fraud_check_label'] = XT_NOVALNET_MOBILE_TEXT;
					$data['fraud_check_value'] = $_SESSION['customer']->customer_payment_address['customers_mobile_phone'];
				} else {
					$data['fraud_check_label'] = XT_NOVALNET_PHONE_TEXT;
					$data['fraud_check_value'] = $_SESSION['customer']->customer_payment_address['customers_phone'];
				}
				$data['show_fraud_module'] = true;
				$_SESSION[$code]['fraud_module_enabled'] = $data['enabled_fraud_module'];
			}
		}
    ...
    public static function get_guarantee_details(&$data, $code)
    {
		...
		if (!empty($xtPlugin->active_modules['xt_novalnet_config']) && $data[$code.'_enable_guarantee_payment']) {
			...
			$order_amount   = $_SESSION['cart']->total['plain'];
			$shipping = new shipping();
			$shipping->_shipping();
			if(!empty($shipping->shipping_data[$_SESSION['selected_shipping']]['shipping_price']['plain'])) {
			  $order_amount += $shipping->shipping_data[$_SESSION['selected_shipping']]['shipping_price']['plain'];
			}
			
			$order_amount = (int) self::get_server_amount_format($order_amount);
			...
			elseif ($is_guarantee && empty($companyvalue)) {
     ...
     * @return boolean
     */
    public static function validate_fraud_amount() {
		$cart_amount = self::get_server_amount_format($_SESSION['cart']->total['plain']);
		if (isset($_SESSION['novalnet']['order_amount']) && $_SESSION['novalnet']['order_amount'] != $cart_amount) {
			self::unset_novalnet_session('novalnet');
            self::checkout_redirect_process(XT_NOVALNET_AMOUNT_CHANGE_ERROR_TEXT);
        }
        return true;
    }
     ...
     *
     */
	public static function validate_pin($input)
	{
     ...
     *
     */
	public static function validate_fraud_data ($code)
	{
        if (empty($_SESSION[$code]['guarantee_payment']) && empty($_SESSION[$code]['guarantee_payment_country_error']) && empty($_SESSION[$code]['guarantee_payment_currency_error']) && empty($_SESSION[$code]['guarantee_payment_address_error']) && empty($_SESSION[$code]['guarantee_payment_mimimum_amount_error']) && !empty($_SESSION[$code]['fraud_module_enabled'])) {

			$_SESSION[$code . '_data'][$code . '_fraud_data'] = trim($_SESSION[$code . '_data'][$code . '_fraud_data']);
			if (empty($_SESSION[$code . '_data'][$code . '_fraud_data']) || !ctype_digit($_SESSION[$code . '_data'][$code . '_fraud_data'])) {
				self::checkout_redirect_process(constant('XT_NOVALNET_EMPTY_' . $_SESSION[$code]['fraud_module_enabled'] . '_FIELD_ERROR_TEXT'));
			}
		}
	}
