...
$(document).ready(
	...
	if($(":input[name*='XT_NOVALNET_PAYPAL_SHOPPING_TYPE']") != undefined) {
		...
		if($('#'+name+'_hint').attr('class') == undefined) {
			$(":input[name*='XT_NOVALNET_PAYPAL_SHOPPING_TYPE']").closest('.x-form-item').after("<div class='x-form-item' tabindex='-1' id="+name+"_hint>"+$('#xt_novalnet_paypal_reference_transaction_hint').val() +"</div>");
		}
	}
